...
pid=$(ps -ef | grep xservice-config | grep java | awk '{print $2}')
if [ ! -z "$pid" ]
then 
  kill -9 $pid
fi
...
-XX:+PrintGCDetails \
-Xms1g -Xmx1g -Xmn690m -Xss256K -XX:MetaspaceSize=128m -XX:MaxMetaspaceSize=512m \
...
-Dconfig=dev \
-jar ${root_path}/xservice-config-fat.jar >/dev/null 2>&1
