...
/**
 * Created by lufei Date : 2017/9/4 11:59 Description :
 */
public class SensitiveWordServiceImpl extends BaseDaoVerticle implements SensitiveWordService {
	...
	public void start(Future<Void> startFuture) throws Exception {
		...
		publishEventBusService(SensitiveWordService.SERVICE_NAME, SensitiveWordService.SERVICE_ADDRESS,
		...
		client = MySQLClient.createNonShared(vertx, mysqlConfig);
	}

	@Override
	public void addSensitiveWord(JsonObject jsonObject, Handler<AsyncResult<JsonObject>> result) {
		JsonArray jsonArray = new JsonArray();
		jsonArray.add(jsonObject.getString("word")).add(1).add(CalendarUtil.format(new Date()))
