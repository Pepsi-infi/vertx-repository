...
/**
 * Created by lufei Date : 2017/8/30 10:57 Description :
 */
public class ImCommonLanguageServiceImpl extends BaseDaoVerticle implements ImCommonLanguageService {
	...
	public interface Sql {
		...
		static final String QUERY_IM_PHRASE_DRIVER = "SELECT * FROM im_phrase_driver WHERE userId = ? ORDER BY createTime";

		static final String QUERY_IM_PHRASE_PASSENGER = "SELECT * FROM im_phrase_passenger WHERE userId = ? ORDER BY createTime";
	...
	public void start(Future<Void> startFuture) throws Exception {
		...
		XProxyHelper.registerService(ImCommonLanguageService.class, vertx, this,
	...
	public void addImCommonLanguage(JsonObject jsonObject, Handler<AsyncResult<JsonObject>> result) {
		...
		jsonArray.add(jsonObject.getString("content")).add(jsonObject.getInteger("weight"))
				.add(jsonObject.getInteger("type"));
	...
	public void queryImCommonLanguage(int type, Integer userId, Handler<AsyncResult<List<JsonObject>>> result) {
		...
		if (type == 1) {
			Future<List<JsonObject>> userFuture = retrieveMany(new JsonArray().add(type), Sql.QUERY_IM_PHRASE_DRIVER);
