...
public class PassengerUnSendServiceImpl extends BaseServiceVerticle implements PassengerUnSendService {
		...
		getFuture.setHandler(res -> {
			if (res.succeeded()) {
				...
				if (CollectionUtils.isEmpty(list)) {
					...
					this.addUnSendMsg2db(requestParam, resultHandler);
				} else {
					logger.info("【调用新增补发消息】需要补发的消息已存在,msgId[" + param.getValue("msgId") + "],phone["
							+ param.getValue("phone") + "],不再新增");
					resultHandler.handle(Future.failedFuture("unSendMsg exists in db"));
				}
			} else {
				logger.error("获取用户未发送出去的消息失败", res.cause());
				resultHandler.handle(Future.failedFuture(res.cause()));
			}
		});
	}
	 ...
	 * @param resultHandler
	 */
	private void addUnSendMsg2db(JsonObject param, Handler<AsyncResult<String>> resultHandler) {
		Future<Integer> addFuture = Future.future();
		this.addUnSendMsg(param, addFuture.completer());
		addFuture.setHandler(handler -> {
			if (handler.succeeded()) {
				logger.info("新增补发消息成功");
				resultHandler.handle(Future.succeededFuture(handler.result().toString()));
			} else {
				logger.error("新增补发消息失败，", handler.cause());
	...
	// 组装新增字段
	private void makeInsertParam(JsonObject param, StringBuilder colStr, StringBuilder positionStr,
			JsonArray positionValue) {
		Set<String> fieldNames = param.fieldNames();
		for (String fieldName : fieldNames) {
			String field = param.getString(fieldName);
			if (StringUtils.isNotBlank(field)) {
