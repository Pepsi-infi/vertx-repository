...
public class MessagePushServiceImpl extends BaseServiceVerticle implements MessagePushService {
	...
	public void start() throws Exception {
		...
		ProxyHelper.registerService(MessagePushService.class, vertx, this, MessagePushService.class.getName());
	...
	public void bisnessMessage(String request,  Handler<AsyncResult<String>> handler) {
		if (StringUtils.isBlank(request)) {
			logger.error("body is null");
			responseError("body is null", handler);
			return;
		}
		// 验证必填项
		...
		if (ResultData.FAIL == checkResult.getCode()) {
			responseError(checkResult.getMsg(), handler);
			return;
		}
		...
		repeatFuture.setHandler(res -> {
			if (res.succeeded()) {
				...
				responseError(res.cause().getMessage(), handler);
			}
		});
	...
	public void responseSuccess(Handler<AsyncResult<String>> resultHandler) {
		ResultData resultData = new ResultData();
		resultHandler.handle(Future.succeededFuture(resultData.toString()));
	}

	public void responseError(String msg, Handler<AsyncResult<String>> resultHandler) {
