...
public class CarSerializer {
	...
	private static int writeObject(ByteArrayOutputStream stream, Object obj, HashMap ht, int hv, String charset) {
		...
		if (obj instanceof Serializable) {
			...
			if (obj instanceof com.ichinait.car.utils.serializer.Serializable) {
				byte[] cs = ((com.ichinait.car.utils.serializer.Serializable) obj).serialize();
				...
				for (int i = 0, len = f.length; i < len; i++) {
					...
					if (Modifier.isPublic(mod)) {
						...
						writeString(stream, getBytes("\0" + getClassName(f[i].getDeclaringClass()) + "\0" + f[i].getName(), charset));
					}
					Object o;
	...
	private static Class getClass(String className) {
		...
		for (int i = 0; i < __packages.length; i++) {
			try {
				Class cls = Class.forName(__packages[i].getName() + "." + className);
	...
	public static Object newInstance(Class cls) {
		...
		for (int i = 0; i < m.length; i++) {
			if (m[i].getReturnType() == cls && Modifier.isStatic(m[i].getModifiers())) {
				try {
	...
	private static UnSerializeResult readCustomObject(ByteArrayInputStream stream, HashMap ht, int hv, String charset) {
		...
		if (o == null) {
			stream.skip(n);
		} else if (o instanceof com.ichinait.car.utils.serializer.Serializable) {
			byte[] b = new byte[n];

			stream.read(b, 0, n);
			((com.ichinait.car.utils.serializer.Serializable) o).unserialize(b);
		} else {
			stream.skip(n);
		}
		stream.skip(1);
