...
public class HttpConsumerVerticle extends AbstractVerticle {
    ...
    private void recivedHttpMessage() {
        ...
        router.route(ServiceUrlConstant.PUSH_MSG_URL).handler(context -> {
            ...
            if (receiveMsg == null) {
                logger.error("body convert json error");
                responseError(resp, "body convert json error");
                return;
            }
     ...
     * @param resultHandler
     */
    private void pushMessage2AndroidDevice(JsonObject receiveMsg, boolean isSocket, Handler<AsyncResult<BaseResponse>> resultHandler) {
