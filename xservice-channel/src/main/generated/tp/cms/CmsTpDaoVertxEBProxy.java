...
public class CmsTpDaoVertxEBProxy implements CmsTpDao {
  ...
  public CmsTpDaoVertxEBProxy(Vertx vertx, String address) {
    this(vertx, address, null);
  }

  public CmsTpDaoVertxEBProxy(Vertx vertx, String address, DeliveryOptions options) {
    this._vertx = vertx;
  ...
  public void getCmsBlockForLang(String blockId, String langCode, Handler<AsyncResult<CmsMutilangDataResponse>> resultHandler) {
    ...
    _deliveryOptions.addHeader("action", "getCmsBlockForLang");
    _vertx.eventBus().<JsonObject>send(_address, _json, _deliveryOptions, res -> {
      if (res.failed()) {
        resultHandler.handle(Future.failedFuture(res.cause()));
      } else {
        resultHandler.handle(Future.succeededFuture(res.result().body() == null ? null : new CmsMutilangDataResponse(res.result().body())));
                      }
    });
  }
  ...
  public void getCMSDataByPageId(long uuid, String platform, String pageId, String langCode, Handler<AsyncResult<String>> result) {
    ...
    JsonObject _json = new JsonObject();
    _json.put("uuid", uuid);
    _json.put("platform", platform);
    _json.put("pageId", pageId);
    _json.put("langCode", langCode);
    DeliveryOptions _deliveryOptions = (_options != null) ? new DeliveryOptions(_options) : new DeliveryOptions();
    _deliveryOptions.addHeader("action", "getCMSDataByPageId");
  ...
  public void getCMSColumnList(String columnId, String langCode, String platform, Handler<AsyncResult<String>> result) {
    ...
    _json.put("platform", platform);
    DeliveryOptions _deliveryOptions = (_options != null) ? new DeliveryOptions(_options) : new DeliveryOptions();
    _deliveryOptions.addHeader("action", "getCMSColumnList");
    _vertx.eventBus().<String>send(_address, _json, _deliveryOptions, res -> {
      if (res.failed()) {
        result.handle(Future.failedFuture(res.cause()));
      } else {
