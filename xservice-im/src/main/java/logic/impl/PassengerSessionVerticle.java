...
public class PassengerSessionVerticle extends AbstractVerticle {

	private static final Logger logger = LoggerFactory.getLogger(PassengerSessionVerticle.class);
	...
	public void start(Future<Void> startFuture) throws Exception {
		CacheManager cacheManager = CacheManagerBuilder.newCacheManagerBuilder()
				.withCache("session",
						CacheConfigurationBuilder.newCacheConfigurationBuilder(String.class, String.class,
								ResourcePoolsBuilder.newResourcePoolsBuilder().offheap(3, MemoryUnit.GB)))
				.withCache("sessionReverse", CacheConfigurationBuilder.newCacheConfigurationBuilder(String.class,
						String.class, ResourcePoolsBuilder.newResourcePoolsBuilder().offheap(3, MemoryUnit.GB)))
				.build(true);
		...
		eb = vertx.eventBus();
		eb.<JsonObject>consumer(PassengerSessionVerticle.class.getName() + innerIP, res -> {
		...
		eb.<JsonObject>consumer(PassengerSessionVerticle.class.getName(), res -> {
			MultiMap headers = res.headers();
			JsonObject body = res.body();
			if (headers != null) {
				String action = headers.get("action");
