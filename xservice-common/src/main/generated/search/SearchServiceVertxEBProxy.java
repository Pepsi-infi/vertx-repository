...
public class SearchServiceVertxEBProxy implements SearchService {
  ...
  public SearchServiceVertxEBProxy(Vertx vertx, String address) {
    this(vertx, address, null);
  }

  public SearchServiceVertxEBProxy(Vertx vertx, String address, DeliveryOptions options) {
    this._vertx = vertx;
  ...
  public void searchTypes(String uuid, SearchRequest request, Handler<AsyncResult<SearchMixResultTp>> resultHandler) {
    ...
    _deliveryOptions.addHeader("action", "searchTypes");
    _vertx.eventBus().<JsonObject>send(_address, _json, _deliveryOptions, res -> {
      if (res.failed()) {
        resultHandler.handle(Future.failedFuture(res.cause()));
      } else {
        resultHandler.handle(Future.succeededFuture(res.result().body() == null ? null : new SearchMixResultTp(res.result().body())));
                      }
    });
  }
  ...
  public void searchAddOnData(String uuid, ParamForSearch params, Integer productId, Integer pageNum, Integer pageSize, String dataType, Handler<AsyncResult<SearchMixResultTp>> resultHandler) {
    ...
    _json.put("pageNum", pageNum);
    _json.put("pageSize", pageSize);
    _json.put("dataType", dataType);
    DeliveryOptions _deliveryOptions = (_options != null) ? new DeliveryOptions(_options) : new DeliveryOptions();
    _deliveryOptions.addHeader("action", "searchAddOnData");
    _vertx.eventBus().<JsonObject>send(_address, _json, _deliveryOptions, res -> {
      if (res.failed()) {
        resultHandler.handle(Future.failedFuture(res.cause()));
      } else {
